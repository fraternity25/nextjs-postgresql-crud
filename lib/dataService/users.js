...
//dynamic version of getUsers
export async function getAllUsersWithFields(fields = []) {
  const userIdsResult = await pool.query(`
    SELECT id FROM users ORDER BY id ASC
  `);

  const users = await Promise.all(
    userIdsResult.rows.map(row => getUserWithFields(row.id, fields))
  );

  return users;
}

//dynamic version of getUserById
export async function getUserWithFields(id, fields = []) {
  ...
  const fieldDependencies = {
    role: ['roles'],
    assigned_tasks: ['tasks', 'creator_users'] 
  };

  // Check JOIN requirements
  const neededJoins = new Set();
  if (fields.length === 0) {
    neededJoins.add('roles');
    neededJoins.add('tasks');
    neededJoins.add('creator_users');
  } 
  else {
    fields.forEach(field => {
      if (fieldDependencies[field]) {
        fieldDependencies[field].forEach(join => neededJoins.add(join));
      }
    });
  }
  ...
  // Build joins
  const joins = [];
  if (neededJoins.has('roles')) {
    joins.push(`LEFT JOIN roles r ON u.role_id = r.id`);
  }
  if (neededJoins.has('tasks')) {
    joins.push(`LEFT JOIN tasks t ON (t.owner_id = u.id OR t.reviewer_id = u.id)`);
  }
  if (neededJoins.has('creator_users')) {
    joins.push(`LEFT JOIN users cu ON t.created_by = cu.id`);
  }

  // Build GROUP BY
  const groupByFields = ['u.id'];
  if (neededJoins.has('roles')) {
    groupByFields.push('r.name'); 
  }
