...
export async function processUsersAndRoles(client, taskId, userIdList, roleList) {
  ...
  if (lenRoles > lenUsers) {
    ...
    const missingUserIds = userIdList.slice(lenRoles); // Get IDs that need roles
    const missingRoles = missingUserIds.map(userId => ({ userId, role: "viewer" }));
    roleList = [...roleList, ...missingRoles];
  }

  //Replace null/undefined roles with "viewer" + ensure userId exists
  roleList = roleList.map((roleObj, index) => {
    const userId = roleObj.userId ?? userIdList[index]; // Fallback to userIdList if missing
    const role = roleObj.role ?? "viewer"; // Replace null/undefined role with "viewer"
    return { userId, role };
  });

  for (let i = 0; i < lenUsers; i++) {
    const userId = userIdList[i];
    const role = roleList[i].role;
