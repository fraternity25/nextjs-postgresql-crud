...
colors.default = colors.info;

export function ToastMessages({messages, type="info"}) {
...
export default function Toast({ messages = [], time = 4000, type="info", onClose }) {
  ...
  const { sliderBg } = colors[type] || colors.default;
  ...
  return (
    <div 
      className={`relative rounded shadow-lg z-50 w-full 
    ...
    >
      {/* Messages */}
      <ToastMessages messages={messages} type={type} />
